# План разработки LLM-ассистента для формулировки целей обучения

## 📊 Отчет по прогрессу

| Итерация | Статус | Дата завершения | Комментарии |
|----------|--------|----------------|-------------|
| 1. Настройка проекта | ✅ Завершена | 2025-09-19 | Структура проекта создана |
| 2. Базовый бот | ✅ Завершена | 2025-09-19 | Бот протестирован, работает |
| 3. LLM интеграция | ✅ Завершена | 2025-09-19 | OpenRouter API готов |
| 4. Система памяти | ✅ Завершена | 2025-09-19 | Контекст диалогов работает |
| 5. Продвинутые функции | ✅ Завершена | 2025-01-27 | Мониторинг и обработка ошибок добавлены |
| 6. Тестирование и деплой | ✅ Завершена | 2025-01-27 | Тесты и Docker готовы |

**Текущий статус:** Завершено  
**Следующий шаг:** Проект готов к использованию  
**Заблокированных задач:** 0

---

## 🚀 Итеративный план разработки

### Итерация 1: Настройка проекта и базовой инфраструктуры
*Результат: Готовая среда разработки с базовой структурой*

- [x] Создать структуру проекта согласно @vision.md (раздел 3)
- [x] Настроить uv и pyproject.toml с основными зависимостями
- [x] Создать базовую конфигурацию в .env.example
- [x] Добавить Makefile с командами сборки и запуска
- [x] Настроить базовое логирование в stdout

**Тест итерации:** `make build` успешно собирает проект

---

### Итерация 2: Базовый Telegram бот
*Результат: Работающий бот с базовыми командами*

- [x] Реализовать src/bot/handlers.py с командами /start и /help
- [x] Настроить src/config/settings.py для загрузки переменных окружения
- [x] Создать src/main.py как точку входа
- [x] Добавить валидацию токена Telegram при запуске
- [x] Реализовать обработку текстовых сообщений (заглушка)

**Тест итерации:** Бот отвечает на /start, /help и принимает текстовые сообщения

---

### Итерация 3: Интеграция с LLM
*Результат: Бот генерирует ответы через OpenRouter API*

- [x] Создать src/llm/client.py для работы с OpenRouter API
- [x] Добавить prompts/system_prompt.txt с базовым промптом
- [x] Реализовать src/llm/prompts.py для загрузки промптов
- [x] Настроить retry-логику и fallback на резервную модель
- [x] Интегрировать LLM ответы в обработчики бота

**Тест итерации:** Бот генерирует осмысленные ответы на вопросы о целях обучения

---

### Итерация 4: Система памяти диалогов  
*Результат: Бот помнит контекст беседы*

- [x] Реализовать src/memory/storage.py с управлением сессиями
- [x] Добавить структуры данных для истории согласно @vision.md (раздел 5)
- [x] Интегрировать историю в запросы к LLM
- [x] Реализовать автоочистку неактивных сессий
- [x] Добавить ограничения на размер истории (10 сообщений)

**Тест итерации:** Бот учитывает предыдущие сообщения в беседе

---

### Итерация 5: Продвинутые функции
*Результат: Полнофункциональный бот с мониторингом*

- [x] Добавить валидацию длины сообщений (1000 символов)
- [x] Реализовать обработку ошибок с user-friendly сообщениями
- [x] Добавить базовые метрики и мониторинг согласно @vision.md (раздел 7)
- [x] Настроить почасовую статистику в логах
- [x] Добавить middleware для обработки исключений

**Тест итерации:** Бот корректно обрабатывает ошибки и ведет статистику

---

### Итерация 6: Тестирование и деплой
*Результат: Протестированный и развернутый бот*

- [x] Создать структуру тестов в tests/ зеркалирующую src/
- [x] Написать базовые unit-тесты для ключевых компонентов
- [x] Добавить Docker конфигурацию согласно @vision.md (раздел 9)
- [x] Настроить деплой на Railway согласно @vision.md (раздел 9)
- [x] Провести интеграционное тестирование с реальными API

**Тест итерации:** Бот работает в продакшене и проходит все тесты

---

## 📋 Дополнительные задачи (по необходимости)

### Оптимизация производительности
- [ ] Оптимизировать время ответа LLM
- [ ] Добавить кэширование частых запросов
- [ ] Настроить логротацию для продакшена

### Улучшение UX
- [ ] Добавить typing indicators во время генерации ответа
- [ ] Улучшить форматирование ответов (markdown)
- [ ] Добавить примеры запросов в /help

### Мониторинг и метрики
- [ ] Расширить систему метрик для детального мониторинга
- [ ] Добавить алерты при превышении лимитов API
- [ ] Настроить backup конфигурации

---

## 🎯 Критерии готовности итераций

1. **Функциональность:** Все чекбоксы итерации выполнены
2. **Тестируемость:** Проходит тест итерации  
3. **Качество кода:** Соответствует принципам из @conventions.md
4. **Документация:** Обновлен прогресс в таблице выше
5. **Готовность к следующей итерации:** Нет критических блокеров

---

## 📚 Справочная информация

- **Техническое видение:** @vision.md
- **Соглашения разработки:** @conventions.md  
- **Идея продукта:** @product_idea.md
- **Общая информация:** @README.md

**Принцип разработки:** Каждая итерация = рабочий результат + новый функционал

## 🔒 Защита токенов от попадания в git

### 1. Создайте файл `.gitignore` в корне проекта:

```bash
# В терминале в папке проекта:
touch .gitignore
```

### 2. Добавьте содержимое в `.gitignore`:

```gitignore
# Секретные файлы с токенами
.env

# Python
__pycache__/
*.py[cod]
*$py.class
*.so
.Python
build/
develop-eggs/
dist/
downloads/
eggs/
.eggs/
lib/
lib64/
parts/
sdist/
var/
wheels/
*.egg-info/
.installed.cfg
*.egg

# Virtual environments
.venv/
venv/
ENV/
env/

# IDE
.vscode/
.idea/
*.swp
*.swo
*~

# OS
.DS_Store
Thumbs.db

# Logs
*.log

# Testing
.pytest_cache/
.coverage
htmlcov/

# uv
uv.lock
```

### 3. Проверьте статус git:

```bash
git status
```

Если `.env` показывается как новый файл, выполните:

```bash
git reset .env  # Убрать из staging area, если добавлен
```

### 4. После создания .gitignore:

```bash
git add .gitignore
git commit -m "Добавлен .gitignore для защиты токенов"
```

### 5. Проверьте что .env игнорируется:

```bash
git status  # .env НЕ должен показываться в списке файлов
```

---

## 🧪 Теперь можете тестировать бота!

После настройки `.gitignore` запустите бота:

```bash
<code_block_to_apply_changes_from>
```

**Бот должен:**
- ✅ Успешно подключиться к Telegram
- ✅ Отвечать на `/start` и `/help`
- ✅ Принимать текстовые сообщения с заглушкой

**Готовы протестировать итерацию 2?**
